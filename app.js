...
const ACTIVITIES = ['إنتاج', 'صيانة', 'إدارة', 'جودة'];
const ACTIVITY_ICONS = { 'إنتاج': '&#127981;', 'جودة': '&#9989;', 'إدارة': '&#128188;', 'صيانة': '&#128295;' };
...
function renderHome() {
  ...
  if (hfc1) hfc1.textContent = allActive.filter(g => g.grade === 'قسم الإنتاج').length;
  if (hfc2) hfc2.textContent = allActive.filter(g => g.grade === 'قسم الصيانة').length;
  if (hfc3) hfc3.textContent = allActive.filter(g => g.grade === 'قسم الإدارة').length;
...
function renderGirlsList() {
  ...
  if (gfc1) gfc1.textContent = activeGirls.filter(g => g.grade === 'قسم الإنتاج').length;
  if (gfc2) gfc2.textContent = activeGirls.filter(g => g.grade === 'قسم الصيانة').length;
  if (gfc3) gfc3.textContent = activeGirls.filter(g => g.grade === 'قسم الإدارة').length;
...
if (DOM.saveGirlBtn) {
  DOM.saveGirlBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!grade) { showToast('الرجاء اختيار السنة الإنتاجة', 'error'); return; }
...
async function saveInlineRating(attKey, rating) {
  ...
  if (!rec) return;
  if (rec.status !== 'حاضر') { showToast('التقييم متاح فقط للحاضرين', 'warning'); return; }
  ...
  const g = state.girls.find(x => x.id === rec.girlId);
  await logHistory('تقييم موظف', `${g?.name || ''} - ${rec.activity} - ${rec.date} - ${rating} نجوم`);
...
function getActivityStats(period, gradeFilter = '', customDate) {
  ...
  const stats = {
    'إنتاج': { present: 0, absent: 0 },
    'جودة': { present: 0, absent: 0 },
    'إدارة': { present: 0, absent: 0 },
    'صيانة': { present: 0, absent: 0 }
  };
...
function renderActivityStats(period, gradeFilter = '') {
  ...
  const icons = { 'إنتاج': '&#127981;', 'جودة': '&#9989;', 'إدارة': '&#128188;', 'صيانة': '&#128295;' };
  const medals = ['&#129351;', '&#129352;', '&#129353;', '4'];
...
if (DOM.exportCSV) {
  DOM.exportCSV.addEventListener('click', () => {
    ...
    if (exportMode === 'month') {
      ...
      wsData.push([]);
      // Header row
      wsData.push(['الاسم', 'السنة', 'إنتاج', 'إدارة', 'جودة', 'صيانة', 'إجمالي الحضور', 'إجمالي الغياب']);
      ...
      exportAtt.forEach(a => {
        if (!grouped[a.girlId]) {
          ...
          grouped[a.girlId] = {
            name: g?.name || '', grade: g?.grade || '',
            'إنتاج': { present: 0, absent: 0 }, 'إدارة': { present: 0, absent: 0 },
            'صيانة': { present: 0, absent: 0 }, 'جودة': { present: 0, absent: 0 },
      ...
      sortedGirls.forEach(r => {
        wsData.push([
          r.name, r.grade,
          r['إنتاج'].present,   // numeric count (not ✓/X)
          r['إدارة'].present,    // numeric count
          r['جودة'].present,   // numeric count
          r['صيانة'].present,  // numeric count
      ...
      detailData.push([]);
      detailData.push(['التاريخ', 'اليوم', 'الموظف', 'السنة', 'النشاط', 'الحالة', 'التقييم', 'ملاحظات']);
      ...
      wsData.push(['الاسم', 'السنة', 'إنتاج', 'إدارة', 'جودة', 'صيانة']);
    ...
    if (exportMode === 'month') {
      ...
      const rows = sortedGirls.map((r, i) => {
        return `<tr>
          ...
          <td>${esc(r.name)}</td>
          <td>${esc(r.grade)}</td>
          <td>${r['إنتاج'].present}</td>
          <td>${r['إدارة'].present}</td>
          <td>${r['جودة'].present}</td>
          <td>${r['صيانة'].present}</td>
      ...
      html = `<!DOCTYPE html><html lang="ar" dir="rtl">
        ...
        <table>
          <tr><th>#</th><th>الاسم</th><th>السنة</th><th>إنتاج</th><th>إدارة</th><th>جودة</th><th>صيانة</th><th>إجمالي الحضور</th><th>إجمالي الغياب</th></tr>
          ${rows}
        </table>
      ...
      html = `<!DOCTYPE html><html lang="ar" dir="rtl">
        ...
        <table>
          <tr><th>#</th><th>الاسم</th><th>السنة</th><th>إنتاج</th><th>إدارة</th><th>جودة</th><th>صيانة</th></tr>
          ${rows}
        </table>
        <div class="footer">تاريخ التصدير: ${new Date().toLocaleDateString('ar-EG')} | نظام الحضور والغياب</div>
        </body></html>`;
